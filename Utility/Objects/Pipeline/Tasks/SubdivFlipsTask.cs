    ...
    public class SubdivFlipsTask : ParallelTask
    {
        public override void Execute(object data)
        {
            if (data is Tuple<List<Flip>, List<Flip>>)
            {
                ...
                var table = Globals.SubdivFlipTotals;
                var table2 = Globals.SubdivFlips;

                foreach (var oldFlip in oldFlips)
                {
                    var subDiv = oldFlip.Before.LegalSubdivisionName;
                    table.AddOrUpdate(
                        subDiv,
                        (x) =>
                        {
                ...
                {
                    var subDiv = newFlip.Before.LegalSubdivisionName;
                    table.AddOrUpdate(
                        subDiv,
                        (x) =>
                        {
